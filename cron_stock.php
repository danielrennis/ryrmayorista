...
// Copiamos SOLO lo que se usa en la web (protegemos cron_stock.php y el .tar)
$archivos_web = ['index.html', 'catalog.json', 'logo.png', 'obtenerimagen.jpeg', 'placeholder_no_image.webp'];
foreach ($archivos_web as $archivo) {
    if (file_exists(__DIR__ . '/' . $archivo)) {
        copy(__DIR__ . '/' . $archivo, $distDir . '/' . $archivo);
    }
}

echo "Iniciando subida rápida a Netlify..." . PHP_EOL;

/**
 * DEPLOY AUTOMÁTICO SEGURO
 * --auth y --site permiten despliegue autónomo sin login manual
 */
$comando = "netlify deploy --dir=dist --prod --auth={$netlify_token} --site={$netlify_site_id} 2>&1";
$output_netlify = shell_exec($comando);

echo "--- RESULTADO NETLIFY ---" . PHP_EOL;
echo $output_netlify;
